...
func (k *KafkaProcessor) processTransaction(msg *ckafka.Message) error {
	...
	createdTransaction, err := transactionUseCase.Register(transaction.AccountID, transaction.Amount, transaction.PixKeyTo, transaction.PixKeyKindTo, transaction.Description)
	if err != nil {
		fmt.Println("error registrering transaction", err)
		return err
	}
